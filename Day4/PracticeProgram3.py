...
# print(l2)
# #or  lazy evaluation
# # l2_map=map(lambda key:emp_data[key].append('test'),emp_data.items())
# # print(next(l2_map))
#
...
# print(max(weather,key=lambda item: list(item.values())[0][1]))
# print(min(weather,key=lambda item: list(item.values())[0][0]))
#
# # 3. Print all the cities that experience min temp more than 30 degree
# print(list(filter(lambda item:list(item.values())[0][0]>30,weather)))
#
# # 4. Create a dictionary to print 'City':'Ave temp'
# avg_temp=dict(map(lambda item: (list(item.keys())[0],(list(item.values())[0][2])),weather))
# print(avg_temp)